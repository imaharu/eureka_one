...
私が本を購入するか選ぶときは、最初の数ページを見てから購入を判断するため全文章からではなく、一定の行数を特徴量として捉えればいいのではないかと考えた。
...
初期段階では、(1)のデータを元データから作成していたのでデータ抽出に時間がかかり、学習に利用するデータが全データの半分ほどしか取ることができなかった。
...
今回の特徴量は単語の頻度に依存したものである。両出版社の文庫とも同じ単語を共通して利用している。

つまり、単純な分離線を引くことが難しいのではないかと考える。

RBFカーネルは、次元を上げることでデータの特徴を捉えることができたため精度がよくなったのではないかと考える。
